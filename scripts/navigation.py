...
from geometry_msgs.msg import (Point, PoseStamped, Quaternion, TwistStamped,
                               Vector3)
from map_manager.srv import GetStaticObstacles, RayCast
from mavros_msgs.msg import PositionTarget, State
from mavros_msgs.srv import (CommandBool, CommandBoolRequest, SetMode,
                             SetModeRequest)
from nav_msgs.msg import Odometry, Path
from navigation_runner.srv import (GetPolicyInference, GetSafeAction,
                                   GetSafeActionMap)
...
class Navigation:
    ...
    def __init__(self, cfg):
        ...
        self.raycast_vres = (
            (self.cfg.sensor.lidar_vfov[1] - self.cfg.sensor.lidar_vfov[0]
             )) / (self.cfg.sensor.lidar_vbeams - 1) * np.pi / 180.0
        ...
        if (self.px4_control):
            ...
            self.state_sub = rospy.Subscriber("/mavros/state", State,
                                              self.state_callback)
            self.action_pub = rospy.Publisher("/mavros/setpoint_raw/local",
                                              PositionTarget,
                                              queue_size=10)
            ...
            self.odom_sub = rospy.Subscriber("/CERLAB/quadcopter/odom",
                                             Odometry, self.odom_callback)
            self.action_pub = rospy.Publisher("/CERLAB/quadcopter/cmd_vel",
                                              TwistStamped,
                                              queue_size=10)
            self.pose_pub = rospy.Publisher("/CERLAB/quadcopter/setpoint_pose",
                                            PoseStamped,
                                            queue_size=10)

        self.goal_sub = rospy.Subscriber("/move_base_simple/goal", PoseStamped,
                                         self.goal_callback)
        self.raycast_vis_pub = rospy.Publisher("/rl_navigation/raycast",
                                               MarkerArray,
                                               queue_size=10)
        self.cmd_vis_pub = rospy.Publisher("/rl_navigation/cmd",
                                           MarkerArray,
                                           queue_size=10)
        self.goal_vis_pub = rospy.Publisher("rl_navigation/goal",
                                            MarkerArray,
                                            queue_size=10)
        self.rollout_traj_pub = rospy.Publisher("/rollout_traj",
                                                Path,
                                                queue_size=10)
        self.dynamic_obstacle_vis_pub = rospy.Publisher(
            "/rl_navigation/in_range_dynamic_obstacles",
            MarkerArray,
            queue_size=10)
    ...
    def init_model(self):
        ...
        observation_spec = CompositeSpec(
            {
                ...
                CompositeSpec({
                    ...
                    CompositeSpec({
                        ...
                        "dynamic_obstacle":
                        UnboundedContinuousTensorSpec(
                            (1, self.cfg.algo.feature_extractor.dyn_obs_num,
        ...
        action_spec = CompositeSpec({
            ...
            CompositeSpec({
                "action":
                UnboundedContinuousTensorSpec((action_dim, ),
                                              device=self.cfg.device),
            })
        }).expand(1, action_dim).to(self.cfg.device)

        policy = PPO(self.cfg.algo, observation_spec, action_spec,
                     self.cfg.device)
    ...
    def takeoff(self):
        ...
        while (not rospy.is_shutdown()
               and not (np.abs(self.odom.pose.pose.position.z - takeoff_height)
                        <= 0.2)):
            if (self.px4_control):
                if (self.mavros_state.mode != "OFFBOARD" and
                    (rospy.Time.now() - last_req) > rospy.Duration(5.0)):
                    if (self.set_mode_client.call(
                            self.offb_set_mode).mode_sent == True):
                    ...
                    if (not self.mavros_state.armed and
                        (rospy.Time.now() - last_req) > rospy.Duration(5.0)):
                        if (self.arming_client.call(
                                self.arm_cmd).success == True):
    ...
    def get_raycast(self, pos: np.array, start_angle: float):
        ...
        try:
            ...
            pos_msg.y = pos[1]
            pos_msg.z = pos[2]
            response = raycast(pos_msg, start_angle,
            ...
            for i in range(num_points):
                p = [
                    response.points[3 * i + 0], response.points[3 * i + 1],
    ...
    def get_dynamic_obstacles(self, pos: np.array):
        dynamic_obstacle_pos = torch.zeros(
            self.cfg.algo.feature_extractor.dyn_obs_num,
            3,
            dtype=torch.float,
            device=self.cfg.device)
        dynamic_obstacle_vel = torch.zeros(
            self.cfg.algo.feature_extractor.dyn_obs_num,
            3,
            dtype=torch.float,
            device=self.cfg.device)
        dynamic_obstacle_size = torch.zeros(
            self.cfg.algo.feature_extractor.dyn_obs_num,
            3,
            dtype=torch.float,
            device=self.cfg.device)
    ...
    def get_static_obstacles(self):
        ...
        try:
            get_static_obstacles_server = rospy.ServiceProxy(
    ...
    def raycast_callback(self, event):
        ...
        start_angle = np.arctan2(self.target_dir[1].cpu().numpy(),
    ...
    def dynamic_obstacle_callback(self, event):
        ...
        dynamic_obstacle_pos, dynamic_obstacle_vel, dynamic_obstacle_size = self.get_dynamic_obstacles(
            pos)
        self.dynamic_obstacles = (dynamic_obstacle_pos, dynamic_obstacle_vel,
    ...
    def goal_callback(self, goal):
        ...
        self.target_dir = torch.tensor([dir_x, dir_y, dir_z],
                                       device=self.cfg.device)
    ...
    def check_obstacle(self, lidar_scan, dyn_obs_states):
        ...
        first_quarter_check, last_quarter_check = torch.all(
            lidar_scan[:, :, :quarter_size,
                       1:] < 0.2), torch.all(lidar_scan[:, :, -quarter_size:,
                                                        1:] < 0.2)
    ...
    def get_safe_action(self, vel_world, action_vel_world):
        ...
        try:
            pos_msg = Point(x=self.odom.pose.pose.position.x,
                            y=self.odom.pose.pose.position.y,
                            z=self.odom.pose.pose.position.z)
            get_safe_action = rospy.ServiceProxy(
                "rl_navigation/get_safe_action", GetSafeAction)
            vel_msg = Vector3(x=vel_world[0].item(),
                              y=vel_world[1].item(),
                              z=vel_world[2].item())
            action_vel_msg = Vector3(x=action_vel_world[0],
                                     y=action_vel_world[1],
                                     z=action_vel_world[2])
            ...
            for i in range(len(self.dynamic_obstacles[0])):
                if (self.dynamic_obstacles[2][i][0] != 0):
                    obs_pos = Vector3(x=self.dynamic_obstacles[0][i][0].item(),
                                      y=self.dynamic_obstacles[0][i][1].item(),
                                      z=self.dynamic_obstacles[0][i][2].item())
                    obs_vel = Vector3(x=self.dynamic_obstacles[1][i][0].item(),
            ...
            safe_action = np.array([
                response.safe_action.x, response.safe_action.y,
    ...
    def get_safe_action_map(self, vel_world, action_vel_world):
        ...
        try:
            pos_msg = Point(x=self.odom.pose.pose.position.x,
                            ...
                            z=self.odom.pose.pose.position.z)
            get_safe_action = rospy.ServiceProxy(
                "rl_navigation/get_safe_action_map", GetSafeActionMap)
            vel_msg = Vector3(x=vel_world[0].item(),
                              y=vel_world[1].item(),
                              z=vel_world[2].item())
            action_vel_msg = Vector3(x=action_vel_world[0],
    ...
    def get_action(self, pos: torch.Tensor, vel: torch.Tensor,
                   goal: torch.Tensor):  # use world velocity
        rpos = goal - pos
        ...
        rpos_clipped_g = vec_to_new_frame(rpos_clipped,
                                          target_dir_2d).squeeze(0).squeeze(0)

        # "relative" velocity
        vel_g = vec_to_new_frame(vel, target_dir_2d).squeeze(0).squeeze(
            0)  # goal velocity

        # drone_state = torch.cat([rpos_clipped, orientation, vel_g], dim=-1).squeeze(1)
        drone_state = torch.cat(
            [rpos_clipped_g, distance_2d, distance_z, vel_g],
            dim=-1).unsqueeze(0)

        # Lidar States
        lidar_scan = torch.tensor(self.raypoints, device=self.cfg.device)
        lidar_scan = (lidar_scan - pos).norm(dim=-1).clamp_max(
            self.cfg.sensor.lidar_range).reshape(1, 1, self.lidar_hbeams,
                                                 self.cfg.sensor.lidar_vbeams)
        ...
        closest_dyn_obs_rpos_g = vec_to_new_frame(
            closest_dyn_obs_rpos.unsqueeze(0), target_dir_2d).squeeze(0)
        closest_dyn_obs_distance = closest_dyn_obs_rpos.norm(dim=-1,
                                                             keepdim=True)
        closest_dyn_obs_distance_2d = closest_dyn_obs_rpos_g[..., :2].norm(
            dim=-1, keepdim=True)
        closest_dyn_obs_distance_z = closest_dyn_obs_rpos_g[...,
                                                            2].unsqueeze(-1)
        closest_dyn_obs_rpos_gn = closest_dyn_obs_rpos_g / closest_dyn_obs_distance.clamp(
            1e-6)

        closest_dyn_obs_vel_g = vec_to_new_frame(
            dynamic_obstacle_vel.unsqueeze(0), target_dir_2d).squeeze(0)

        obs_res = 0.25
        closest_dyn_obs_width = torch.max(dynamic_obstacle_size[:, 0],
                                          dynamic_obstacle_size[:, 1])
        ...
        closest_dyn_obs_height[(closest_dyn_obs_height <= 1)
                               & (closest_dyn_obs_height != 0)] = 1.
        ...
        if (has_obstacle_in_range):
            if (not self.use_policy_server):
                ...
                try:
                    get_policy_inference = rospy.ServiceProxy(
                        "rl_navigation/GetPolicyInference", GetPolicyInference)

                    response = get_policy_inference(
                        obs["agents"]["observation"]
                        ["state"].cpu().numpy().flatten().tolist(),
                        obs["agents"]["observation"]["state"].size(),
                        obs["agents"]["observation"]
                        ["lidar"].cpu().numpy().flatten().tolist(),
                        obs["agents"]["observation"]["lidar"].size(),
                        obs["agents"]["observation"]
                        ["direction"].cpu().numpy().flatten().tolist(),
                        obs["agents"]["observation"]["direction"].size(),
                        obs["agents"]["observation"]["dynamic_obstacle"].cpu(
                        ).numpy().flatten().tolist(), obs["agents"]
                        ["observation"]["dynamic_obstacle"].size())
            ...
            vel_world = (goal - pos) / torch.norm(
                goal - pos) * self.cfg.algo.actor.action_limit
        return vel_world

    def get_rollout_traj(self,
                         pos: torch.Tensor,
                         vel: torch.Tensor,
                         goal: torch.Tensor,
                         dt=0.1,
    ...
    def control_callback(self, event):
        ...
        if (not self.goal_received or len(self.raypoints) == 0
                or len(self.dynamic_obstacles) == 0):
        ...
        # check for angle
        goal_angle = np.arctan2(self.target_dir[1].cpu().numpy(),
                                self.target_dir[0].cpu().numpy())
        ...
        orientation = torch.tensor([
            ...
            self.odom.pose.pose.orientation.z
                                   ...
                                   device=self.cfg.device)
        rot = self.quaternion_to_rotation_matrix(
        ...
        vel_world = torch.tensor(rot @ vel_body,
                                 device=self.cfg.device,
                                 dtype=torch.float)  # world vel

        # get RL action from model
        cmd_vel_world = self.get_action(
            pos, vel_world, goal).squeeze(0).squeeze(0).detach().cpu().numpy()
        ...
        quat_no_tilt = tf.transformations.quaternion_from_euler(
            0, 0, curr_angle)
        quat_msg = Quaternion()
        ...
        if (distance <= 3. and distance > 0.3):
            if (np.linalg.norm(safe_cmd_vel_local) != 0):
                safe_cmd_vel_local = 0.5 * safe_cmd_vel_local / np.linalg.norm(
                    safe_cmd_vel_local)
                safe_cmd_vel_world = 0.5 * safe_cmd_vel_world / np.linalg.norm(
                    safe_cmd_vel_world)
        elif (distance <= 1.0):
    ...
    def run(self):
        raycast_timer = rospy.Timer(rospy.Duration(0.05),
                                    self.raycast_callback)
        raycast_vis_timer = rospy.Timer(rospy.Duration(0.05),
                                        self.raycast_vis_callback)
        control_timer = rospy.Timer(rospy.Duration(0.05),
                                    self.control_callback)
        goal_vis_timer = rospy.Timer(rospy.Duration(0.05),
                                     self.goal_vis_callback)
        dynamic_obstacle_timer = rospy.Timer(rospy.Duration(0.05),
                                             self.dynamic_obstacle_callback)
        dynamic_obstacle_vis_timer = rospy.Timer(
            rospy.Duration(0.05), self.dynamic_obstacle_vis_callback)
        cmd_vis_timer = rospy.Timer(rospy.Duration(0.05),
                                    self.cmd_vis_callback)
