...
class policy_server:
    ...
    def handle_inference(self, req):
        ...
        drone_state = torch.tensor(req.state, device=self.cfg.device).view(
            req.state_shape)
        lidar_scan = torch.tensor(req.lidar,
                                  device=self.cfg.device).view(req.lidar_shape)
        target_dir_2d = torch.tensor(
            req.direction, device=self.cfg.device).view(req.direction_shape)
        dyn_obs_states = torch.tensor(req.dynamic_obstacle,
                                      device=self.cfg.device).view(
                                          req.dynamic_obstacle_shape)
        ...
        with set_exploration_type(ExplorationType.MEAN):
            output = self.policy(
                obs)  # Modify according to your model's inference method
        vel_world = output["agents", "action"]
        # Return the result
        # print(vel_world.cpu().detach().squeeze(0).squeeze(0).numpy().tolist())
        return {
            'action':
            vel_world.cpu().detach().squeeze(0).squeeze(0).numpy().tolist()
        }

    def start_server(self):
        rospy.Service('rl_navigation/GetPolicyInference', GetPolicyInference,
