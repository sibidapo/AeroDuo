...
from .utils import (GAE, BetaActor, IndependentBeta, ValueNorm, make_mlp,
                    vec_to_world)
...
class PPO(TensorDictModuleBase):
    ...
    def __init__(self, cfg, observation_spec, action_spec, device):
        ...
        dynamic_obstacle_network = nn.Sequential(
            Rearrange("n c w h -> n (c w h)"), make_mlp([128,
                                                         64])).to(self.device)
        ...
        self.feature_extractor = TensorDictSequential(
            ...
            CatTensors([
                "_cnn_feature",
                ("agents", "observation", "state"), "_dynamic_obstacle_feature"
            ],
                       "_feature",
                       del_keys=False),
        ...
        self.actor = ProbabilisticActor(
            TensorDictModule(BetaActor(self.action_dim), ["_feature"],
                             ["alpha", "beta"]),
            in_keys=["alpha", "beta"],
            out_keys=[("agents", "action_normalized")],
            distribution_class=IndependentBeta,
            return_log_prob=True).to(self.device)

        # Critic network
        self.critic = TensorDictModule(nn.LazyLinear(1), ["_feature"],
    ...
    def __call__(self, tensordict):
        ...
        actions = (2 * tensordict["agents", "action_normalized"] *
                   self.cfg.actor.action_limit) - self.cfg.actor.action_limit
