...
class SimState:
    ...
    def __init__(
        ...
        self.index = index
        self.step = step
        self.raw_trajectory_info = copy.deepcopy(
            raw_trajectory_info)  # GT PATH
    ...
    def pose(self):  #
        return self.trajectory[-1]['sensors']['state'][
            'position'] + self.trajectory[-1]['sensors']['state']['orientation']
...
class ENV:
    ...
    def get_obs_at(self, index: int, state: SimState):
        ...
        if args.run_type in ['collect', 'train']:
            teacher_action_path = get_waypoint_at(STEP_NUM=7,
                                                  DISTANCE=1,
                                                  state=state)
            done = state.is_end
        elif args.run_type in ['eval']:
...
def get_waypoint_at(STEP_NUM: int, DISTANCE: int, state: SimState):
    ...
    delta_unit = (np.array(end_point) -
                  np.array(predict_xyz)) / (np.linalg.norm(
                      (np.array(end_point) - np.array(predict_xyz))) + 1e-8)
