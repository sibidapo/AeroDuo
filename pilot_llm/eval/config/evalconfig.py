...
class CommonArguments:
    ...
    gpu_id: int = field(default=0, metadata={"help": "simulator gpus"})
    ...
    stage1_ckpt: Optional[str] = field(default=None, metadata={"help": "Path to Stage 1 trainable_state.pt"})
    stage2_ckpt: Optional[str] = field(default=None, metadata={"help": "Path to Stage 2 trainable_state.pt"})
    no_zgraph: bool = field(default=False, metadata={"help": "Stage 2 checkpoint was trained standalone (--no_zgraph): no z_graph conditioning, Stage 1 / SAM2 / GroundingDINO are never loaded"})

    steps_per_plan: int = field(default=2, metadata={"help": "number of predicted waypoints executed per plan step"})
