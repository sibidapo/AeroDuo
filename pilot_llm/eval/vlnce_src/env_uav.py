...
from config.evalconfig import args
...
def project_target_state2global_state_axis(this_target_state, target_state):
    ...
    this_global_pos = np.linalg.inv(rot).T @ np.array(this_pos) + np.array(
        start_pos)
    this_global_eular = np.array(this_eular) + np.array(start_eular)
    return {
        'position': this_global_pos.tolist(),
        'orientation': this_global_eular.tolist()
    }
...
def prepare_object_map():
    ...
    map_dict = {
        'Carla_Town01': 'Carla_Town01_test',
        'Carla_Town02': 'Carla_Town02_test',
        'Carla_Town03': 'Carla_Town03_test',
        'Carla_Town04': 'Carla_Town04_test',
        'Carla_Town05': 'Carla_Town05_test',
        'Carla_Town06': 'Carla_Town06_test',
        'Carla_Town07': 'Carla_Town07_test',
        'Carla_Town10HD': 'Carla_Town10HD_test',
        'Carla_Town15': 'Carla_Town15_test',
        'ModernCityMap': 'ModernCityMap_test',
        'NewYorkCity': 'NewYorkCity_test',
        'ModularPark': 'ModularPark_test',
        'NYCEnvironmentMegapa': 'NYC_test',
        'TropicalIsland': 'TropicalIsland_test'
    }
...
class AirVLNENV:
    ...
    def __init__(self,
        ...
        self.sim_states: Optional[List[SimState]] = [
        ...
        self.pos_queue = deque(maxlen=10)
    ...
    def load_my_datasets(self, path):
        ...
        for traj_path in tqdm.tqdm(trajectorys_path):
            ...
            if (len(self.activate_maps) > 0
                    and map_name not in self.activate_maps
                ) or seq_name in skipped_trajectory_set:
                continue
            ...
            with open(mark_json, 'r') as f:
                ...
                _, closest_area_info = find_closest_area(
                    object_position, self.map_area_dict[map_name])
                object_position = [
                    closest_area_info[9], closest_area_info[10],
                    closest_area_info[11]
                ]
                obj_pose = airsim.Pose(
                    airsim.Vector3r(closest_area_info[9],
                                    closest_area_info[10],
                                    closest_area_info[11]),
                    airsim.Quaternionr(closest_area_info[13],
                                       closest_area_info[14],
                                       closest_area_info[15],
                                       closest_area_info[12]))
                obj_scale = airsim.Vector3r(closest_area_info[17],
                                            closest_area_info[17],
                                            closest_area_info[17])
            ...
            if not os.path.exists(os.path.join(traj_path,
                                               'gt_waypoints.json')):
                invalid_cnt += 1
                continue
            ...
            with open(
                    os.path.join(traj_path,
                                 'object_description_with_help.json')) as f:
    ...
    def _group_scenes(self):
        ...
        return sorted(self.data,
                      key=lambda e: (scene_sort_keys[str(e['map_name'])]))
    ...
    def _changeEnv(self, need_change: bool = True):
        ...
        for index, item in enumerate(machines_info_template):
            ...
            delta = min(self.batch_size, item['MAX_SCENE_NUM'],
    ...
    def _setTrajectorys(self):
        ...
        drone1_start_position_list = [
            item['drone1_start'] for item in self.batch
        ]
        drone2_start_position_list = [
            item['drone2_start'] for item in self.batch
        ]
        ...
        for index_1, item in enumerate(self.machines_info):
            ...
            for index_2, _ in enumerate(item['open_scenes']):
                pose = airsim.Pose(position_val=airsim.Vector3r(
                ...
                pose2 = airsim.Pose(position_val=airsim.Vector3r(
        ...
        for _ in range(3):
            ...
            results2 = self.simulator_tool.setPoses(poses=poses2,
        ...
        for index_1, item in enumerate(self.machines_info):
            for index_2, _ in enumerate(item['open_scenes']):
                self.sim_states[cnt] = SimState(
                    index=cnt, step=0, raw_trajectory_info=self.batch[cnt])
                self.sim_states[cnt].trajectory = [
                    state_info_results[index_1][index_2]
                ]
                self.sim_states[cnt].drone2_traj = [
                    state_info_results[index_1][index_2]['sensors']['state2']
                ]
                cnt += 1
    ...
    def _getStates(self):
        ...
        for index_1, item in enumerate(self.machines_info):
            for index_2 in range(len(item['open_scenes'])):
                ...
                states[cnt] = (rgb_images, depth_images, state, bev_images,
                               bev_depth)
                cnt += 1
        return states
    ...
    # def makeActions(self, waypoint1, waypoint2, navigator: Navigator,
    #                 traj_status):
    def simple_action_drone1(self, pos: np.ndarray,
                             goal: np.ndarray) -> np.ndarray:
        vel_world = (goal - pos) / np.linalg.norm(goal - pos) * 5.0
        assert vel_world.shape == (3,), 'error shape'
        return vel_world

    def fly_drone1_leg(self, goal, drone2_goal, airsim_client,
                       vehicle_name="Drone_1"):
        """Fly one time-budgeted leg toward an absolute waypoint.

        The leg's total cost is bounded up front by its length: velocity
        commands are issued in fixed slices toward the goal, and the leg ends
        on arrival, on exhausting the time budget, or after two consecutive
        slices without meaningful progress (blocked). No unbounded retries.
        Drone2 is advanced one paced step per slice.

        Returns True if the leg ended blocked (no progress).
        """
        CRUISE_SPEED = 2.0
        ARRIVE_RADIUS = 3.0   # velocity control stops ~1-2 m short; match drone2
        SLICE_SECONDS = 5.0

        target = np.array(goal[:3], dtype=np.float64)
        target_yaw_deg = np.degrees(goal[3])  # predicted heading, honored directly

        kin = airsim_client.getMultirotorState(
            vehicle_name=vehicle_name).kinematics_estimated
        pos = np.array([p_i for p_i in kin.position])
        total_dist = np.linalg.norm(target - pos)

        if total_dist < ARRIVE_RADIUS:
            airsim_client.simPause(False)
            if total_dist > 0.1:
                # Micro-move: fly the small delta directly in one 1 s command
                # so short waypoints still translate (a zero-motion leg would
                # trip eval's <0.1 m consecutive-point collision check).
                delta = target - pos
                airsim_client.moveByVelocityAsync(
                    ...
                    yaw_mode=airsim.YawMode(is_rate=False,
                                            yaw_or_rate=target_yaw_deg),
                    vehicle_name=vehicle_name,
                ).join()
            else:
                # No translation needed — still rotate to the commanded heading.
                airsim_client.rotateToYawAsync(target_yaw_deg, timeout_sec=3,
                                               margin=5,
                                               vehicle_name=vehicle_name).join()

        budget = total_dist / CRUISE_SPEED * 1.5 + 5.0
        elapsed = 0.0
        no_progress = 0
        blocked = False
        ...
        while elapsed < budget:
            ...
            duration = min(SLICE_SECONDS, max(dist / CRUISE_SPEED, 0.5))

            self.move_drone2(drone2_goal, airsim_client, duration=duration)
            if self.drone2_collision:
                break

            vel = (target - pos) / dist * CRUISE_SPEED
            airsim_client.simPause(False)
            airsim_client.moveByVelocityAsync(
                float(vel[0]), float(vel[1]), float(vel[2]), duration=duration,
                drivetrain=airsim.DrivetrainType.MaxDegreeOfFreedom,
            ...
            kin = airsim_client.getMultirotorState(
                vehicle_name=vehicle_name).kinematics_estimated
            new_pos = np.array([p_i for p_i in kin.position])
            if np.linalg.norm(new_pos - pos) < 0.3 * CRUISE_SPEED * duration:
                no_progress += 1
                if no_progress >= 2:
                    print('Drone1 leg blocked: no progress for 2 slices')
                    blocked = True
                    break
            else:
                no_progress = 0

        # Skip the brake if drone2's stuck check paused the sim — a join on a
        # command issued while paused never completes.
    ...
    def move_drone2(self, goal, airsim_client, vehicle_name="Drone_2",
        ...
        airsim_client.simSetKinematics(new_state, ignore_collision=True,
                                       vehicle_name=vehicle_name)
        airsim_client.simContinueForFrames(1)

        pos = np.array([new_state.position.x_val,
                        new_state.position.y_val, height])
        self.drone2_pos_queue.append(pos)

        goal = np.asarray(goal, dtype=np.float64)
        direction = goal[:2] - pos[:2]
        dist2 = np.linalg.norm(direction)

        # Arrival radius: drone2 stops ~1-2 m short of its waypoint under
        ...
        if len(self.drone2_pos_queue) == self.drone2_pos_queue.maxlen:
            ...
            if np.linalg.norm(history_loc - pos) < 0.1:
                print('Drone2 move on path api: stuck max len')
                self.drone2_collision = True
                airsim_client.simPause(True)
                return pos

        fly_time = max(0.1, min(float(duration), dist2 / 5.0))
    ...
    def navigate(self, waypoint1, waypoint2):

        airsim_client = self.simulator_tool.airsim_clients[0][0]
        ...
        airsim_client.enableApiControl(True, vehicle_name="Drone_1")
        airsim_client.armDisarm(True, vehicle_name="Drone_1")

        ##TODO: implement if self.activatedrone2 for ablation

        self.drone2_pos_queue.clear()
        self.drone2_collision = False
        airsim_client.enableApiControl(True, vehicle_name="Drone_2")
        airsim_client.armDisarm(True, vehicle_name="Drone_2")

        airsim_client.simPause(True)
        start_time = airsim_client.getMultirotorState(vehicle_name="Drone_1").timestamp*1e-10
        ...
        # Only count collisions newer than this leg (has_collided is sticky).
        pre_coll_time = airsim_client.simGetCollisionInfo(
            vehicle_name="Drone_1").time_stamp

        blocked = self.fly_drone1_leg(waypoint1[0], waypoint2[0], airsim_client)

        airsim_collision_flag = False
        if blocked:
            self.collision = True
            coll_info = airsim_client.simGetCollisionInfo(vehicle_name="Drone_1")
            airsim_collision_flag = (coll_info.has_collided
                                     and coll_info.time_stamp > pre_coll_time)

        end_time = airsim_client.getMultirotorState(vehicle_name="Drone_1").timestamp * 1e-10
        end_pos = airsim_client.getMultirotorState(vehicle_name="Drone_1").kinematics_estimated.position
        ...
        return {
            'states': results,
            'drone2_states': state_info2,
            'collision': (self.collision, self.drone2_collision),
            'airsim_collision': airsim_collision_flag
        }, delta_time, delta_distance, end_pos
        


    def makeActions(self, waypoint1, waypoint2, traj_status):
        
        results, delta_time, delta_distance, end_pos = self.navigate(waypoint1, waypoint2)

        if results is None:
            raise Exception('navigate failed, results is None')
        
        states = results['states']
        drone2_states = results['drone2_states']
        drone1_collision, drone2_collision = results['collision']
        ...
        for index, waypoint in enumerate(waypoint1):
            ...
            self.sim_states[index].is_collisioned = drone1_collision
            self.sim_states[index].drone2_traj.append(drone2_states)
        
        self.update_measurements()
        return drone1_collision, drone2_collision, delta_time, delta_distance, end_pos, airsim_collision


    def Teleport(self, waypoint1, waypoint2):
        airsim_client: MultirotorClient = self.simulator_tool.airsim_clients[
            0][0]
        state_sensor = State(airsim_client, drone_name='Drone_1')
        imu_sensor = Imu(airsim_client, imu_name='Imu', drone_name='Drone_1')
        drone2_state_sensor = State(airsim_client, drone_name='Drone_2')
        ...
        if len(waypoint1) > 0:
            start_pos = airsim_client.getMultirotorState(
                vehicle_name="Drone_1").kinematics_estimated.position
            start_pos = np.array([p_i for p_i in start_pos])
            ...
            target_pose = airsim.Pose(
                ...
                airsim.to_quaternion(0, 0, math.radians(target_yaw)))
            airsim_client.moveByVelocityBodyFrameAsync(0, 0, 0, 0.02)
            airsim_client.simSetVehiclePose(target_pose,
                                            True,
                                            vehicle_name="Drone_1")
        ...
        airsim_client.moveByVelocityBodyFrameAsync(0, 0, 0, 0.02)
        airsim_client.simSetVehiclePose(drone2_pose,
                                        True,
                                        vehicle_name="Drone_2")
    ...
    def _update_distance_to_target(self):
        ...
        for idx, target_position in enumerate(target_positions):
            ...
            current_drone2_position = self.sim_states[idx].drone2_traj[-1][
                'position']
    ...
    def _updata_Success(self):
        for i, state in enumerate(self.sim_states):
            ...
            if (self.sim_states[i].is_end and distance_to_target
                    <= self.sim_states[i].SUCCESS_DISTANCE):
    ...
    def _updata_NDTW(self):
        ...
        def euclidean_distance(
            ...
            return np.linalg.norm(np.array(position_b) - np.array(position_a),
                                  ord=2)
        ...
        for i, state in enumerate(self.sim_states):
            ...
            if len(state.NDTW['locations']) == 0:
                ...
                if current_position.tolist(
                ) == state.NDTW['locations'][-1].tolist():
                    continue
                self.sim_states[i].NDTW['locations'].append(current_position)

            dtw_distance = fastdtw.fastdtw(
                self.sim_states[i].NDTW['locations'],
                self.sim_states[i].NDTW['gt_locations'],
                dist=euclidean_distance)[0]

            nDTW = np.exp(-dtw_distance /
                          (len(self.sim_states[i].NDTW['gt_locations']) *
                           self.sim_states[i].SUCCESS_DISTANCE))
    ...
    def _update_OracleSuccess(self):
        for i, state in enumerate(self.sim_states):
            ...
            self.sim_states[i].OracleSuccess['_metric'] = float(
                self.sim_states[i].OracleSuccess['_metric']
                or d <= self.sim_states[i].SUCCESS_DISTANCE)
