...
import transformers
from transformers import AutoProcessor, SchedulerType, get_scheduler
...
def main():
    ...
    # Enable gradient checkpointing to dramatically reduce memory footprint
    model.gradient_checkpointing_enable()

    # load pretrain model (seg and depth pretraining)
    if args.reload_model_path is not None and args.reload_model_path != "None":
    ...
    for n, p in model.named_parameters():
        if any(
            [
                x in n
                for x in ["mask_head", "depth_head", "mask_query"]
            ]
        ):
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if accelerator.is_main_process:
                ...
                wandb.log({"Loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}, step=completed_steps)
