...
class GraphEncoder(nn.Module):
    ...
    def _forward_single(
        ...
        return x_dict['position']   # [T, D_g]

    # ── Forward ───────────────────────────────────────────────────────────────

    def forward(
        self,
        place_nodes: torch.Tensor,           # [T, D_g]  or  [B, T, D_g]
        observation_vertices: torch.Tensor,   # [T, K_max, D_g]  or  [B, T, K_max, D_g]
    ) -> torch.Tensor:                        # [T, D_g]  or  [B, T, D_g]
        """
        Run HGT message passing and return the enriched position-node sequence.

        Accepts both single-episode (place_nodes.dim()==2) and batched
        (place_nodes.dim()==3) inputs.  For B > 1, the B independent graphs are
        fused into one disconnected super-graph via torch_geometric.data.Batch
        so all HGTConv layers execute in a single kernel call.
        """
        if place_nodes.dim() == 2:
            return self._forward_single(place_nodes, observation_vertices)

        # Batched path ─────────────────────────────────────────────────────────
        from torch_geometric.data import Batch

        B, T = place_nodes.shape[:2]
        graphs = [
            self._build_graph(place_nodes[b], observation_vertices[b])
            for b in range(B)
        ]
        batched = Batch.from_data_list(graphs)

        x_dict = {
            'position':    batched['position'].x,
            'observation': batched['observation'].x,
        }
        edge_index_dict = {
            ('position',    'temporal',    'position'):    batched['position', 'temporal',    'position'].edge_index,
            ('position',    'observes',    'observation'): batched['position', 'observes',    'observation'].edge_index,
            ('observation', 'observed_by', 'position'):   batched['observation', 'observed_by', 'position'].edge_index,
        }
        ...
        # Each episode contributes exactly T position nodes; reshape → [B, T, D_g]
        return x_dict['position'].reshape(B, T, self.D_g)
